<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!doctype html>
<html lang="en">
<head>
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<title>Example for Request Object</title>
</head>
<body>
	<nav class="navbar fixed-top navbar-dark bg-dark">
  		<a class="navbar-brand" href="index.jsp">
		    <img src="data:image/png;base64," class="d-inline-block align-top" alt="">
		    <% out.print(application.getAttribute("title")); %>
		  </a>
		  <%Integer count = (Integer) session.getAttribute("count");if(count != null && count > 0){ %>
			<div class="pull-right navbar-brand">
				Item : <%=count%>
			</div>
		<%} %>
		<button type="button" class="btn btn-light pull-right" onclick="javascript:history.go(-1)">Back</button>
	</nav>
	<div class="container" style="padding-top:60px;padding-bottom:60px;">
	  <div class="row pt-2">
	  	<% 
	  		pageContext.setAttribute("title", "Application JSP", pageContext.APPLICATION_SCOPE);
	  		pageContext.setAttribute("title", "Page JSP", pageContext.PAGE_SCOPE);
	  		pageContext.setAttribute("title", "Request JSP", pageContext.REQUEST_SCOPE);
	  		pageContext.setAttribute("title", "Session JSP", pageContext.SESSION_SCOPE);
	  	%>
	  	<div class="card border-success mb-3">
		  <div class="card-header bg-transparent border-success">PageContext Object</div>
		  <div class="card-body text-success">
		  		Displaying Application scope  : <% out.println(pageContext.getAttribute("title",pageContext.APPLICATION_SCOPE)); %> <br/>
				Displaying Page scope : <% out.println(pageContext.getAttribute("title",pageContext.PAGE_SCOPE));  %> <br/>
				Displaying Request scope : <% out.println(pageContext.getAttribute("title",pageContext.REQUEST_SCOPE));  %> <br/>
				Displaying Session scope : <% out.println(pageContext.getAttribute("title",pageContext.SESSION_SCOPE));  %> <br/>
		  </div>
		</div>
	  </div>
  	</div>
	<nav class="navbar fixed-bottom navbar-dark bg-dark justify-content-center">
  		<a class="navbar-brand" href="javascript:void(0)">Thank you</a>
	</nav>
</body>
</html>